    ...
    for (const customer of customers) {
      ...
      if (customer.favorites && customer.favorites.length > 0) {
        ...
        const cleanedFavorites = customer.favorites.filter(favId => 
          validFavoriteIds.some(validId => validId.toString() === favId.toString())
        );
        
        if (cleanedFavorites.length !== originalFavoritesCount) {
          console.log(`🧹 ${customer.username}: Removed ${originalFavoritesCount - cleanedFavorites.length} orphaned favorite products`);
      ...
      if (customer.following && customer.following.length > 0) {
        ...
        const cleanedFollowing = customer.following.filter(followId => 
          validFollowingIds.some(validId => validId.toString() === followId.toString())
        );
        
        if (cleanedFollowing.length !== originalFollowingCount) {
          console.log(`🧹 ${customer.username}: Removed ${originalFollowingCount - cleanedFollowing.length} orphaned followed stores`);
