...
async function investigateDiscrepancy() {
  try {
    ...
    console.log(
      `Database favorites array length: ${customer.favorites.length}`
    );
    ...
    console.log("=".repeat(50));
    console.log(
      `Database following array length: ${customer.following.length}`
    );
    ...
    console.log("=".repeat(50));
    console.log(
      `Database says: ${customer.favorites.length} favorites, ${customer.following.length} following`
    );
    console.log(
      `Actually exist: ${validFavorites} favorites, ${validFollowing} following`
    );
    ...
    if (validFavorites !== customer.favorites.length) {
      console.log(
        `\n⚠️  ISSUE FOUND: ${
          customer.favorites.length - validFavorites
        } favorite products don't exist anymore`
      );
    }
    ...
    if (validFollowing !== customer.following.length) {
      console.log(
        `\n⚠️  ISSUE FOUND: ${
          customer.following.length - validFollowing
        } followed stores don't exist anymore`
      );
    }
    ...
    console.log("=".repeat(50));
    console.log(
      "Update the API to only count favorites/following that still exist:"
    );
