...
async function testMongoDBConnection() {
  ...
  console.log(
    "📍 Connection URI:",
    MONGODB_URI.replace(/%40Jenzon0108_/, "***")
  );
  ...
  try {
    ...
    try {
      ...
      console.log(
        `   Total found: ${await db.collection("customers").countDocuments()}`
      );
    ...
    try {
      ...
      console.log("\n🏪 MSMEs:");
      console.log(
        `   Total found: ${await db.collection("msmes").countDocuments()}`
      );
      if (msmes.length > 0) {
    ...
    try {
      ...
      console.log("\n👨‍💼 ADMINS:");
      console.log(
        `   Total found: ${await db.collection("admins").countDocuments()}`
      );
      if (admins.length > 0) {
    ...
    try {
      ...
      console.log("\n🛍️ PRODUCTS:");
      console.log(
        `   Total found: ${await db.collection("products").countDocuments()}`
      );
    ...
    try {
      ...
      console.log("\n⭐ REVIEWS:");
      console.log(
        `   Total found: ${await db.collection("reviews").countDocuments()}`
      );
      if (reviews.length > 0) {
    ...
    try {
      ...
      console.log("\n🏆 BADGES:");
      console.log(
        `   Total found: ${await db.collection("badges").countDocuments()}`
      );
      if (badges.length > 0) {
